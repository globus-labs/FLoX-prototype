        ...
        if keras_dataset not in available_datasets:
            ...
            if num_samples:
                idx = np.random.choice(
                    np.arange(len(x_train)), num_samples, replace=True
                )
                x_train = x_train[idx]
                y_train = y_train[idx]
...
def federated_learning(
    ...
    for i in range(loops):
        ...
        for e, num_s, num_epoch, path_d in zip(
            endpoint_ids, num_samples, epochs, path_dir
        ):
            tasks.append(
                fx.submit(
                    training_function,
        ...
        if aggregation_mode == "average":
            ...
            raise Exception(
                f"Federated mode {aggregation_mode} is not recognized. \
        ...
        if (
            ...
            and evaluation_function
            and callable(evaluation_function)
        ):
